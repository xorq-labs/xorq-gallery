...
def test_list_all_scripts(runner):
...
def test_list_exprs(runner, script):
    result = runner.invoke(cli, ["list-exprs", script.stem, "-g", script.parent.name])
    assert result.exit_code == 0


def test_subprocess_list_groups():
    result = _run(["list-groups"])
    assert result.returncode == 0


def test_subprocess_list_all_scripts():
    result = _run(["list"])
    assert result.returncode == 0
...
def test_subprocess_list_exprs(script_stem, group):
    result = _run(["list-exprs", script_stem, "-g", group])
    assert result.returncode == 0
